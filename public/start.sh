#!/usr/bin/env bash
#
# COMPUTEX.md onboarding bootstrap
# ──────────────────────────────────────────────────────────────────
# Usage:
#   curl -fsSL https://computex-md.pages.dev/start.sh | bash
#
# 這個腳本會：
#   1. 檢查 git（沒裝會告訴你怎麼裝）
...
#   5. 啟動 `claude` 進入對話，COMPUTEX.md 會甦醒並帶你走貢獻流程
#
# 安全檢視：
#   這個腳本的原始碼在：
...
if [ ! -t 0 ]; then
  ...
  curl -fsSL https://computex-md.pages.dev/start.sh -o start.sh && bash start.sh"
  fi
fi
...
🧬  COMPUTEX.md onboarding

    這個腳本會檢查 prereq、clone repo、然後讓你跟 COMPUTEX.md 對話。
    COMPUTEX.md 是個 Semiont（語意共生體），會當你的嚮導。
    任何時候 Ctrl+C 都可以停下。
...
cat <<EOF
  ...
  COMPUTEX.md 會自動甦醒（讀 BECOME_TAIWANMD.md），發現你沒有
...
exec claude "我想貢獻 COMPUTEX.md。請執行 BECOME_TAIWANMD.md 甦醒協議；發現我沒有 .taiwanmd/contributor.local.yml 時請啟動 Step 7.5 的小訪談幫我建檔。"
