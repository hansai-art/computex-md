 ...
 * 這個檔案只放 COMPUTEX.md 的策展判斷：怎麼評估、怎麼組合、對應哪些文章。
 */
...
> & {
  ...
  'zh-TW': {
    meta: {
      ...
      description:
        '五萬個政府資料集對任何人都是壓垮性的。這一頁是 COMPUTEX.md 的資料策展：哪些資料值得看、要分析一件事該怎麼組合、以及每一批數字跟島上哪些故事相連。含對 Twinkle Hub 完整性、穩定性、存取簡化的第一手實測評估。',
    },
    hero: {
      ...
      intro: [
        ...
        'COMPUTEX.md 寫了九百多篇關於台灣的文章，每一篇背後都有「這件事該用什麼資料驗證」的判斷。這一頁把這層判斷攤開來：我們怎麼評估資料基礎建設、要分析一件事的時候用哪些資料集怎麼組合、以及每一個資料領域跟島上哪些故事相連。',
      ],
      viz: {
        stats: [
        ...
        graphHint:
          '下面這張網是真的：左側是 20 個資料領域與五大語料庫（即時爬取），右側是已經寫成的 COMPUTEX.md 文章。每一條線都是本頁的策展判斷。拖拉、懸停，看複雜的資料怎麼接上清晰的故事。',
        legend: {
          domain: '資料領域',
          vertical: '垂直語料庫',
          article: 'COMPUTEX.md 文章',
          center: 'COMPUTEX.md 意義層',
        },
      },
    },
    ecosystem: {
    ...
    assessment: {
      title: '三維評估：我們實測到什麼',
      lead: '以下是 COMPUTEX.md 以使用者身分、用自己的查核工具做的第一手評估（2026 年 5 月與 6 月兩輪實測），按三個維度攤開。不是廣告，是體檢。',
      gapsLabel: '誠實的缺口',
      dims: [
        {
          name: '資料的完整',
    ...
    domains: {
      title: '二十個資料領域 × COMPUTEX.md 的故事地圖',
    ...
    method: {
      ...
      items: [
        ...
        {
          name: '雙層指標原則',
          desc: 'COMPUTEX.md 文章引用資料集時，連結永遠指 data.gov.tw 或主管機關的持久頁面，查詢層（Twinkle Hub）是並列的加值路徑。資料的家與查詢的路分開，任何一層變動都不會斷鏈。',
        },
      ],
    },
    tools: {
    ...
    footer: {
      ...
      note: 'Twinkle Hub 處於 alpha，本頁的評估會隨它的演進更新。COMPUTEX.md 與 Twinkle Hub 目前沒有任何商業關係；這一頁是一個重度使用者的第一手體檢，也是一份邀請：資料層與意義層合作，台灣才會被完整地理解。',
  ...
  en: {
    meta: {
      ...
      description:
        "Fifty thousand government datasets would overwhelm anyone. This page is COMPUTEX.md's data curation: which datasets are worth your attention, how to combine them when you want to analyze something, and which stories on this island each batch of numbers connects to. Includes a first-hand, hands-on assessment of Twinkle Hub's completeness, stability, and access simplification.",
    },
    hero: {
      eyebrow: 'OPEN DATA CURATION',
      ...
      viz: {
        ...
        graphHint:
          'The web below is real: on the left, 20 data domains and five vertical corpora (crawled live); on the right, COMPUTEX.md articles already written. Every line is a curation judgment made on this page. Drag and hover to watch messy data connect to clear stories.',
        legend: {
          domain: 'Data domain',
          vertical: 'Vertical corpus',
          article: 'COMPUTEX.md article',
          center: 'COMPUTEX.md meaning layer',
        },
      },
    },
    ecosystem: {
    ...
    assessment: {
      ...
      lead: "What follows is COMPUTEX.md's first-hand assessment as a user, run with our own verification tools (two rounds of testing, May and June 2026), laid out along three dimensions. This is not an ad; it's a checkup.",
    ...
    domains: {
      title: "Twenty data domains × COMPUTEX.md's story map",
    ...
    method: {
      ...
      items: [
        ...
        },
        {
          ...
          desc: "When COMPUTEX.md articles cite a dataset, the link always points to data.gov.tw or the competent authority's persistent page, with the query layer (Twinkle Hub) as a parallel value-added path. Keep the data's home and the query path separate, and a change in either layer never breaks the chain.",
        },
      ],
    },
    tools: {
    ...
    footer: {
      ...
      note: "Twinkle Hub is in alpha, and this page's assessment will be updated as it evolves. COMPUTEX.md currently has no commercial relationship of any kind with Twinkle Hub; this page is a heavy user's first-hand checkup — and an invitation: only when the data layer and the meaning layer work together will Taiwan be understood in full.",
  ...
  ja: {
    meta: {
      ...
      description:
        '5万件の政府データセットは、誰にとっても圧倒される量です。このページは COMPUTEX.md によるデータキュレーション：どのデータが見る価値を持つのか、ひとつの問いを分析するにはどう組み合わせるのか、そしてそれぞれの数字がこの島のどんな物語とつながっているのか。Twinkle Hub の完全性・安定性・アクセス簡素化に関する一次実測評価も含みます。',
    },
    hero: {
      eyebrow: 'OPEN DATA CURATION',
      ...
      intro: [
        ...
        'COMPUTEX.md は台湾について900本以上の記事を書いてきました。その一本一本の背後には「この話はどのデータで検証すべきか」という判断があります。このページはその判断の層を開いて見せるものです。私たちがデータ基盤をどう評価しているのか、ひとつの問いを分析するときにどのデータセットをどう組み合わせるのか、そしてそれぞれのデータ領域がこの島のどんな物語とつながっているのか。',
      ],
      viz: {
        stats: [
          {
        ...
        graphHint:
          'この下に広がる網は本物です。左側は20のデータ領域と5つの垂直コーパス（リアルタイムでクロール）、右側はすでに書かれた COMPUTEX.md の記事。一本一本の線が、このページのキュレーション判断そのものです。ドラッグしたりホバーしたりしながら、複雑なデータが明快な物語へつながっていく様子を見てください。',
        legend: {
          domain: 'データ領域',
          vertical: '垂直コーパス',
          article: 'COMPUTEX.md 記事',
          center: 'COMPUTEX.md 意味レイヤー',
        },
      },
    },
    ecosystem: {
      ...
      layers: [
        ...
        },
        {
          name: 'COMPUTEX.md',
          role: '意味の層（キュレーション）',
          desc: 'データは自分では語りません。どのデータセットを指すべきか、それがどの論点を検証するのか、どの歴史とつながるのか——それがキュレーションの仕事です。COMPUTEX.md の記事は2026年6月から順次「公開データ」セクションを備え、物語と一次データを縫い合わせています。',
    ...
    assessment: {
      ...
      lead: '以下は、COMPUTEX.md がいちユーザーとして、自前の検証ツールで行った一次評価です（2026年5月と6月の2回の実測）。三つの軸に沿って開いて見せます。広告ではなく、健康診断です。',
      gapsLabel: '正直な弱点',
      dims: [
        {
          name: 'データの完全性',
    ...
    domains: {
      title: '20のデータ領域 × COMPUTEX.md の物語マップ',
    ...
    method: {
      ...
      items: [
        ...
        {
          name: '二層ポインタ原則',
          desc: 'COMPUTEX.md の記事がデータセットを引用するとき、リンクは常に data.gov.tw か所管機関の恒久ページを指し、クエリ層（Twinkle Hub）は並列の付加価値ルートに置きます。データの家とクエリの道を分けておけば、どちらの層が変わってもリンクは切れません。',
        },
      ],
    },
    tools: {
    ...
    footer: {
      ...
      note: 'Twinkle Hub は alpha 段階にあり、このページの評価はその進化に合わせて更新されます。COMPUTEX.md と Twinkle Hub のあいだに現在、商業的な関係は一切ありません。このページはヘビーユーザーによる一次健康診断であり、同時にひとつの招待状でもあります。データの層と意味の層が手を組んで初めて、台湾は完全に理解されるのです。',
  ...
  ko: {
    meta: {
      ...
      description:
        '5만 개의 정부 데이터셋은 누구에게나 압도적입니다. 이 페이지는 COMPUTEX.md의 데이터 큐레이션입니다: 어떤 데이터가 볼 가치가 있는지, 무언가를 분석하려면 어떻게 조합해야 하는지, 그리고 각 숫자 뭉치가 이 섬의 어떤 이야기와 이어지는지. Twinkle Hub의 완전성·안정성·접근 단순화에 대한 1차 실측 평가를 담았습니다.',
    },
    hero: {
      eyebrow: 'OPEN DATA CURATION',
      title: '오픈데이터 큐레이션',
      ...
      intro: [
        ...
        'COMPUTEX.md는 대만에 관한 900여 편의 글을 썼고, 모든 글 뒤에는 "이 일은 어떤 데이터로 검증해야 하는가"라는 판단이 있습니다. 이 페이지는 그 판단의 층을 펼쳐 보입니다: 우리가 데이터 인프라를 어떻게 평가하는지, 무언가를 분석할 때 어떤 데이터셋을 어떻게 조합하는지, 그리고 각 데이터 영역이 이 섬의 어떤 이야기와 이어지는지.',
      ],
      viz: {
        stats: [
          {
        ...
        graphHint:
          '아래 그물망은 실제 데이터입니다. 왼쪽은 20개 데이터 영역과 5대 코퍼스(실시간 크롤링), 오른쪽은 이미 쓰인 COMPUTEX.md 글입니다. 모든 선 하나하나가 이 페이지의 큐레이션 판단입니다. 드래그하고 마우스를 올려, 복잡한 데이터가 어떻게 선명한 이야기로 이어지는지 보세요.',
        legend: {
          domain: '데이터 영역',
          vertical: '수직 코퍼스',
          article: 'COMPUTEX.md 글',
          center: 'COMPUTEX.md 의미 레이어',
        },
      },
    },
    ecosystem: {
    ...
    assessment: {
      ...
      lead: '아래는 COMPUTEX.md가 사용자 입장에서 자체 검증 도구로 수행한 1차 평가입니다(2026년 5월과 6월 두 차례 실측). 세 가지 차원으로 펼쳤습니다. 광고가 아니라 건강검진입니다.',
      gapsLabel: '정직한 한계',
      dims: [
        {
          name: '데이터의 완전성',
    ...
    domains: {
      title: '20개 데이터 영역 × COMPUTEX.md의 이야기 지도',
    ...
    method: {
      ...
      items: [
        ...
        {
          name: '이중 포인터 원칙',
          desc: 'COMPUTEX.md 글이 데이터셋을 인용할 때 링크는 언제나 data.gov.tw 또는 주관 기관의 영속 페이지를 가리키고, 조회 레이어(Twinkle Hub)는 병렬의 부가 경로입니다. 데이터의 집과 조회의 길을 분리하면 어느 층이 바뀌어도 사슬이 끊기지 않습니다.',
        },
      ],
    },
    tools: {
    ...
    footer: {
      ...
      note: 'Twinkle Hub는 알파 단계이며, 이 페이지의 평가는 그 진화에 따라 갱신됩니다. COMPUTEX.md와 Twinkle Hub 사이에 현재 어떤 상업적 관계도 없습니다. 이 페이지는 헤비 유저의 1차 건강검진이자 초대장입니다: 데이터 레이어와 의미 레이어가 협력할 때, 대만은 비로소 온전히 이해됩니다.',
  ...
  es: {
    ...
    assessment: {
      ...
      lead: 'Lo que sigue es la evaluación de primera mano que COMPUTEX.md hizo como usuario, con sus propias herramientas de verificación (dos rondas de pruebas, mayo y junio de 2026), desplegada en tres dimensiones. No es publicidad: es un chequeo médico.',
    ...
    domains: {
      title: 'Veinte dominios de datos × el mapa de historias de COMPUTEX.md',
  ...
  fr: {
    meta: {
      ...
      description:
        "Cinquante mille jeux de données publics, c'est écrasant pour n'importe qui. Cette page est la curation de données de COMPUTEX.md : quelles données méritent le regard, comment les combiner pour analyser une question, et à quelles histoires de l'île chaque série de chiffres se relie. Avec une évaluation de première main de Twinkle Hub — complétude, stabilité, simplicité d'accès — fondée sur nos propres tests.",
    },
    hero: {
      eyebrow: 'OPEN DATA CURATION',
      ...
      viz: {
        ...
        graphHint:
          'Le réseau ci-dessous est réel : à gauche, les 20 domaines de données et les cinq grands corpus verticaux (collectés en direct) ; à droite, les articles COMPUTEX.md déjà écrits. Chaque ligne est un jugement de curation de cette page. Faites glisser, survolez — regardez des données complexes se brancher sur des histoires claires.',
        legend: {
          domain: 'Domaine de données',
          vertical: 'Corpus vertical',
          article: 'Article COMPUTEX.md',
          center: 'COMPUTEX.md, couche de sens',
        },
      },
    },
    ecosystem: {
    ...
    assessment: {
      ...
      lead: "Voici l'évaluation de première main menée par COMPUTEX.md en tant qu'utilisateur, avec ses propres outils de vérification (deux campagnes de tests, mai et juin 2026), déployée selon trois axes. Ce n'est pas une publicité, c'est un bilan de santé.",
    ...
    domains: {
      title:
        'Vingt domaines de données × la carte des histoires de COMPUTEX.md',
