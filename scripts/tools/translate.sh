...
# Function to check if we're in a COMPUTEX.md repository
check_repo() {
...
setup_repo() {
    print_info "檢查 COMPUTEX.md 環境..."
    
    if check_repo; then
        print_success "已在 COMPUTEX.md 目錄內"
        return 0
    fi
    
    # Not in the right directory, try to clone
    print_warning "不在 COMPUTEX.md 目錄內，準備 clone repository..."
...
get_translation_prompt() {
    ...
    if [[ -f "docs/prompts/TRANSLATE_PROMPT.md" ]]; then
        ...
        cat << EOF
# COMPUTEX.md Translation Prompt

> 把這整段貼到 ChatGPT / Claude / Gemini，它會幫你翻譯一篇 COMPUTEX.md 文章。

## 你的角色
你是 COMPUTEX.md 的翻譯志工。COMPUTEX.md 是一個用 Markdown 策展台灣知識的開源專案。
...
Created by COMPUTEX.md Translation Automation System."
...
main() {
    ...
    echo -e "${PURPLE}${MAGIC}     🇹🇼 COMPUTEX.md 翻譯貢獻自動化系統 🌐                   ${MAGIC}${NC}"
    echo -e "${PURPLE}${MAGIC}                                                           ${MAGIC}${NC}"
    ...
    if [[ $processed_count -gt 0 ]]; then
        ...
        echo ""
        print_magic "感謝你為 COMPUTEX.md 貢獻翻譯！🙏"
