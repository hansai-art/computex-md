...
Dimensions:
  ...
  3. Template description — description 開頭是「COMPUTEX.md / 開源台灣 / 台灣的」等品牌詞
...
# Astro template adds " | COMPUTEX.md" suffix to frontmatter title, so the
        ...
        if t_cjk < TITLE_MIN_CJK:
            yield Violation(
                ...
                fix_suggestion=(
                    ...
                    " Astro template 會自動加 ' | COMPUTEX.md' suffix，所以 frontmatter title 短主題名（4-10字）是 OK 的。"
                ),
            )
        ...
        if brand_match:
            yield Violation(
                ...
                fix_suggestion=(
                    ...
                    " 開頭給「具體 scene / 數字 / 矛盾」，不是「COMPUTEX.md / 這篇文章」這種 meta 詞。"
                ),
            )
