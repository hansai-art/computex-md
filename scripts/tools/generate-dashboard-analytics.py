...
def clean_title(title: str) -> str:
    return (title or "").replace(" | COMPUTEX.md", "").strip()
...
def _is_brand_query(q_str):
    """Classify a query as brand (contains 'computex.md' / 'taiwan md' / exact 'md' / 'computexmd').

    2026-04-17 δ: SC 7d 總 CTR 8.54% 虛胖 — top queries 'taiwan md' 62% / 'computex.md' 71%
...
def build_sc_7d_section(sc_raw):
    ...
    return {
        ...
        "brandBreakdown": {
            ...
            "note": (
                "Brand = queries containing 'computex.md' / 'taiwan md' / exact 'md' / "
                "'computexmd'. Total CTR aggregates both; brand CTR is usually 60-80%, "
...
def build_ai_crawlers_dashboard(ai_raw):
    ...
    # 多少 COMPUTEX.md — 翻譯 infrastructure 的真正 KPI。Pass-through，cache 有才有。
